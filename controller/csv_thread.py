...
class CSVThread(threading.Thread):
    ...
    def run(self):
        print(f"--- {self.switch.name} CSV verileri dinleniyor... ---")
        while True:
            try:
                # # data al
                ...
                data = self.controller.q_feature.get()
                if not data:
                    continue
                ...
                with open(self.csv_file, 'a') as f:
                    # Eğer dosya yeni oluşturulmuşsa başlık ekle
                    if f.tell() == 0:
                        f.write("flow_id,flow_iat_min,flow_iat_max,flow_iat_mean,flow_iat_std,rate_ratio,max_packet_length,min_packet_length,packet_length_mean\n")
                    # Veriyi CSV formatında yaz
                    f.write(f"{data['flow_id']},{data['features'][0]},{data['features'][1]},{data['features'][2]},{data['features'][3]},{data['features'][4]},{data['features'][5]},{data['features'][6]},{data['features'][7]}\n")

...
class SwitchConnection(object):
    ...
    def WriteDigestEntry(self, digest_entry, dry_run=False):
        ...
        if dry_run:
            print("P4Runtime Write Digest:", request)
        else:
            self.client_stub.Write(request)
    ...
    def Digest(self, dry_run=False):
        request = self.dispatcher.digest_queue.get()
        if dry_run:
            print("P4 Runtime Digest (dry run)")
        else:
            return request
